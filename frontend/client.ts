 ...
 */
import { exportCanvas as api_design_export_canvas_exportCanvas } from "~backend/design/export-canvas";
import { uploadImage as api_design_upload_image_uploadImage } from "~backend/design/upload-image";
...
export namespace design {
    ...
    export class ServiceClient {
         ...
         */
        public async exportCanvas(params: RequestType<typeof api_design_export_canvas_exportCanvas>): Promise<ResponseType<typeof api_design_export_canvas_exportCanvas>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/canvas/export`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_design_export_canvas_exportCanvas>
        }

        /**
         * Uploads an image to object storage and returns the public URL.
         */
        public async uploadImage(params: RequestType<typeof api_design_upload_image_uploadImage>): Promise<ResponseType<typeof api_design_upload_image_uploadImage>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/images/upload`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_design_upload_image_uploadImage>
        }
    }
}
